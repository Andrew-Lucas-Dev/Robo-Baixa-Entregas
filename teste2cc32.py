    ...
    with sqlite3.connect('banco_dados_entregas2.db') as conexao:
        ...
        for nota in lista_notas:
            try:
                ...
                cursor.execute(''' 
                INSERT INTO notas2 (filial,serie, nota, data_nota_fiscal, data_chegada, data_entrega, data_descarreg, status,cod_oco, MDFe, num_carga, CC, baixado)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,'NAO')''', 
                (filial,serie, numero_nota, data_nota, data_chegada, data_entrega, data_descarreg, status,cod_oco, man, num_carga, cc))
                ...
                if status == 'Entregue' and baixado == 'NAO':
                    ...
                    SET 
                        ...
                        baixado = '?',
                    WHERE nota = ? AND MDFe = ?''', 
                    (filial,serie, data_nota, data_chegada, data_entrega, data_descarreg, status,cod_oco,man, num_carga, numero_nota, man, cc, baixado))
...
def extrair_mdf_e(mdf_e):
    ...
    if pd.isna(mdf_e):
        return None
    match = re.search(r"(?:1/2|2/1|2/2|/5/1|5/2|3/2|3/U|3/13|3/18)/([\d.]+)", str(mdf_e))
...
for index, row in Planilha_CC32.iterrows():
    ...
    try:
        ...
        nota_dict = {
            ...
            'cod_oco': row.get('Cod Ocorrencia', 'Pendente'),
            'man': row.get('MDF-e'),
...
# for i, linha in enumerate(Planilha_CC32.index):
#     nf = Planilha_CC32.loc[linha, "NF"]
#     serie = Planilha_CC32.loc[linha, "Série"]
#     cte = Planilha_CC32.loc[linha, "Ct-e/OST"]
#     carga = Planilha_CC32.loc[linha, "Carga"]
#     man = Planilha_CC32.loc[linha, "MDF-e"]
#     status = Planilha_CC32.loc[linha, "STATUS"]
#     status_observacao = Planilha_CC32.loc[linha, "Cod Observação"]
#     data_nota_fiscal = Planilha_CC32.loc[linha, "DATA NOTA FISCAL"]
#     data_chegada = Planilha_CC32.loc[linha, "Data Chegada"]   
#     data_entrega = Planilha_CC32.loc[linha, "Data Entrega"]   
#     data_fim_descarregamento =  Planilha_CC32.loc[linha, "Data Fim Descarreg."] 
#     hor_chegada = Planilha_CC32.loc[linha, "Horario Chegada"]  
#     hor_entrega = Planilha_CC32.loc[linha, "Horario Entrega"]  
#     hor_fim_descarregamento = Planilha_CC32.loc[linha, "Horario Fim Descarreg."] 
#     codobservacao = Planilha_CC32.loc[linha, "Cod Observação"] 
#     atendente = Planilha_CC32.loc[linha, "Atendente"]
...
#     if status_observacao in Planilha_Observação['DESCRICAO OBSERVACAO'].values:
#         cod_observacao = Planilha_Observação.loc[Planilha_Observação[Planilha_Observação['DESCRICAO OBSERVACAO'] == status_observacao].index.values, 'COD OBS'].values[0]
#     else:
...
#     print(f'nf:{nf} carga:{carga} man:{man} status:{status} data:{data_chegada}')
...
#         for i in range(10):
#             pyautogui.press("backspace")
#         pyautogui.write(str(data_chegada + hor_chegada))
#         pyautogui.sleep(time)
#         pyautogui.press('tab')
#         if ocorrencia == 1:
#             try:
#                 pyautogui.write(str(data_entrega + hor_entrega))
#                 pyautogui.sleep(time)
#                 pyautogui.press('tab')
#                 pyautogui.write(str(data_fim_descarregamento + hor_fim_descarregamento))
#             except:
#                 pyautogui.write(str(data_chegada + hor_chegada_dev))
#                 pyautogui.sleep(time)
#                 pyautogui.press('tab')
#                 pyautogui.write(str(data_chegada + hor_chegada_dev2))
